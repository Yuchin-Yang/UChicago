...
class ResourceSim(threading.Thread):
  ...
  def select_next_server(self):
    ...
    current_server = self.lastServer
    self.lastServer += 1
    if self.lastServer == self.numServers:
      self.lastServer = 0
    ...
    # current_server = 0 
    # for index in range(self.gui.getSampleSize()):
    #   if self.servers[index].get_current_load() < self.servers[current_server].get_current_load():
