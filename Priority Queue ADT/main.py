...
def test_queue(q, name):
  ...
  q.enqueue( 15, "Remove pencil sharpened shavings" )
...
def time_few_priorities( q, name ):
  """Test how long it takes to complete the operations"""
  print("--Testing time when there are few priorities.--")
  start = time.time()
  for i in range( 10000 ):             # the number of items
...
def time_interleaved_enq_deq( q, name, percentage ):
  """Test how long it takes to complete the operations"""
  print("--Testing time when enqueue and dequeue are interleaved.--")
  frequecy = 100 // percentage
  start = time.time()
  for i in range( 10000 ):             # the number of items
    p = random.randint( 0, 10 )      # priority values
    v = random.randint( 0, 10000 )   # some random values
    q.enqueue( p, v )
    if i % frequecy == 0:            # Regularly dequeue
      q.dequeue()
  end = time.time()
  print( "Time to enqueue 10000 items and dequeue in the", name, "is {0:.8f} us.".format( end - start ) )

def time_enqueue_before_dequeue( q, name ):
  """Test how long it takes to complete the operations"""
  print("--Testing time to first enqueue all and then dequeue.--")
  start = time.time()
  for i in range( 10000 ):             # the number of items
    p = random.randint( 0, 10 )      # priority values
    v = random.randint( 0, 10000 )   # some random values
  ...
  print( "Time to enqueue 10000 items and then dequeue in the", name, "is {0:.8f} us.".format( end - start ) )
