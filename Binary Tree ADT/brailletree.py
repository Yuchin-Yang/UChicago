...
class BrailleTree:
  ...
  def _check_braille(self, braille_string,node):
    '''
      ...
      Parameters:
        ...
        braille_string; a string of 6 0's and 1's representing braille cells.
    ...
    '''
    #TODO
    if len(braille_string) == 0:
      return node, braille_string
    if braille_string[0]=="1":
      if node.right is None:
        return node, braille_string
      else:
        return self._check_braille(braille_string[1:],node.right)
    else:
      if node.left is None:
        return node, braille_string
      else:
        return self._check_braille(braille_string[1:],node.left)
    ...
    if len(braille_string) == 0:
      ...
      if braille_string[0]== "1":
       #create a node 
        ...
        new_node = Node(level)
        node.right = new_node
        self._add_value(braille_string[1:],new_node,val)      
      else:
        #create a node 
        level= node.data + 1
        new_node = Node(level)
  ...
  def write_values(self, filepath):
    ...
    paths = []
    data = []
    self.traverse(paths, data, '', self.root)
    for i in range(len(paths)):
      f.write(paths[i] + ': ' + data[i] + '\n')
    f.close()

  def traverse(self, paths, data, path, node):
    if len(path) == 6:
      paths.append(path)
    ...
    if node.left is not None:
      self.traverse(paths, data, path +'0', node.left)

    if node.right is not None:
      self.traverse(paths, data, path +'1', node.right)
